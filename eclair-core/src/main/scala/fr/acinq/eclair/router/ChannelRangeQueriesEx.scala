    ...
    format match {
      case UNCOMPRESSED_FORMAT =>
        shortChannelIds.foreach(id => {
          ...
          Protocol.writeUInt32(timestamp(id), bos, ByteOrder.BIG_ENDIAN)
        })
      case ZLIB_FORMAT =>
        val output = new DeflaterOutputStream(bos)
        shortChannelIds.foreach(id => {
          Protocol.writeUInt64(id.toLong, output, ByteOrder.BIG_ENDIAN)
          Protocol.writeUInt32(timestamp(id), output, ByteOrder.BIG_ENDIAN)
        })
        output.finish()
    }
    bos.toByteArray
  ...
  def encodeShortChannelIdAndFlagSingle(shortChannelIds: Iterable[ShortChannelId], flag: ShortChannelId => Byte, format: Byte): BinaryData = {
    ...
    format match {
      ...
      case ZLIB_FORMAT =>
        ...
        shortChannelIds.foreach(id => {
          Protocol.writeUInt64(id.toLong, output, ByteOrder.BIG_ENDIAN)
          Protocol.writeUInt8(flag(id), output)
        })
        output.finish()
    }
    bos.toByteArray
